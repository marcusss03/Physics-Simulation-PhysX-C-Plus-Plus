...
namespace VisualDebugger
{
	...
	static std::chrono::steady_clock::time_point gTimerStart;
	static float gTimeRemaining = 30.0f;
	...
	void Init(const char *window_name, int width, int height)
	{
		...
		auto bladeBox = scene->GetBulldozer();
		auto bladeDyn = static_cast<physx::PxRigidDynamic*>(bladeBox->Get());
		physx::PxVec3 bladePos = bladeDyn->getGlobalPose().p;
		physx::PxVec3 eye = bladePos + physx::PxVec3(0, 2, 6);
		physx::PxVec3 dir = (bladePos - eye).getNormalized();

		camera = new Camera(eye, dir, 5.0f);
		...
		//mera = new Camera(PxVec3(0.0f, 5.0f, 15.0f), PxVec3(0.f,-.1f,-1.f), 5.f);
	...
	void RenderScene()
	{
		...
		if (!scene->Pause() && gTimeRemaining > 0.0f) {
			gTimeRemaining -= delta_time;                   // count down
			if (gTimeRemaining < 0.0f) gTimeRemaining = 0;  // clamp
		}
		...
		char buf[32];
		std::snprintf(buf, sizeof(buf), "Time: %2d s", int(std::ceil(gTimeRemaining)));

		Renderer::RenderText(std::string(buf),PxVec2(0.50f, 0.95f),PxVec3(0.f, 0.f, 0.f),0.018f);

		Renderer::RenderText(std::string("W/A/S/D = move   Q/Z = up/down   Mouse+click = look    Arrow Keys = Bulldozer Movement"),PxVec2(0.02f, 0.03f),PxVec3(0.f, 0.f, 0.f),0.018f);

		//Points
		std::ostringstream ss;
		ss << "Score: " << scene->GetScore() << std::endl << "Target: 300";

		Renderer::RenderText(ss.str(), physx::PxVec2(0.05f, 0.90f), physx::PxVec3(0.f, 0.f, 0.f), 0.018f);

		if (scene->GetScore() >= scene->GetTarget())
		{
			//Stop sim
			scene->Pause(true);
			...
			const PxVec2 pos(0.35f, 0.55f);
			const PxVec3 col(0.0f, 0.0f, 0.0f); 

			//Win message
			Renderer::RenderText(msg, pos, col, sz);
		}

		Renderer::Finish();
		if (hud_show) hud.Render();
